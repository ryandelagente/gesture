...
class BugController extends Controller
{
    ...
    public function bulk(Request $request)
    {
        ...
        $data = $request->validate([
            'action' => 'required|in:status,assignee,priority,delete',
            'ids'    => 'required|array|min:1',
            'ids.*'  => 'integer|exists:bugs,id',
            'value'  => 'nullable',
        ]);

        $bugs = Bug::whereIn('id', $data['ids'])
            ->forWorkspace($user->current_workspace_id)
            ->get();
        ...
        foreach ($bugs as $bug) {
            if ($data['action'] === 'delete') {
                if ($bug->canBeDeletedBy($user)) {
                    $bug->delete();
                    $updated++;
                }
                continue;
            }
            if (!$bug->canBeUpdatedBy($user)) {
                continue;
            }
            switch ($data['action']) {
                case 'status':
                    $bug->bug_status_id = (int) $data['value'];
                    break;
                case 'assignee':
                    $bug->assigned_to = $data['value'] ? (int) $data['value'] : null;
                    break;
                case 'priority':
                    if (in_array($data['value'], ['low', 'medium', 'high', 'critical'])) {
                        $bug->priority = $data['value'];
                    }
                    break;
            }
            $bug->save();
    ...
    public function exportCsv(Request $request)
    {
        ...
        $query = Bug::with(['project', 'bugStatus', 'assignedTo', 'reportedBy', 'tags'])
            ->forWorkspace($user->current_workspace_id)
            ->accessibleByUser($user, $userWorkspaceRole);

        if ($request->project_id)   { $query->forProject($request->project_id); }
        if ($request->status_id)    { $query->byStatus($request->status_id); }
        if ($request->priority)     { $query->byPriority($request->priority); }
        if ($request->severity)     { $query->bySeverity($request->severity); }
        if ($request->assigned_to)  { $query->where('assigned_to', $request->assigned_to); }
        if ($request->search)       { $query->where('title', 'like', '%' . $request->search . '%'); }

        $filename = 'bugs-' . now()->format('Y-m-d-His') . '.csv';

        return response()->streamDownload(function () use ($query) {
            $out = fopen('php://output', 'w');
            fputs($out, "\xEF\xBB\xBF"); // UTF-8 BOM for Excel compatibility
            fputcsv($out, [
                'id', 'title', 'project', 'status', 'priority', 'severity',
                'assigned_to', 'reported_by_user', 'reporter_guest_name', 'reporter_guest_email',
                'source', 'tags', 'page_url', 'browser', 'os', 'viewport',
            ...
            $query->orderBy('id')->chunk(500, function ($rows) use ($out) {
                foreach ($rows as $bug) {
                    fputcsv($out, [
                        ...
                        $bug->guest_email ?? '',
                        $bug->source,
                        $bug->tags->pluck('name')->implode(', '),
                        $bug->page_url ?? '',
                        $bug->browser ?? '',
                        $bug->os ?? '',
                        $bug->viewport_w ? ($bug->viewport_w . 'x' . $bug->viewport_h) : '',
                        $bug->created_at?->format('Y-m-d H:i:s'),
                        $bug->updated_at?->format('Y-m-d H:i:s'),
                    ]);
                }
            });
            fclose($out);
